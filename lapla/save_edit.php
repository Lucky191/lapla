...
////////Изменение имени
 ...
 if (isset($_POST['name'])){//Если существует имя
    ...
    $up = mysqli_query($conn,"UPDATE users SET name_user='$name' WHERE id='$id_user'");//обновляем имя
    if ($up == true) {
...
////////Изменение фамилии
    ...
    $up = mysqli_query($conn,"UPDATE users SET lastname='$lastname' WHERE id='$id_user'");//обновляем фамилию
...
////////Изменение города
    ...
    $up = mysqli_query($conn,"UPDATE users SET phone='$phone' WHERE id='$id_user'");//обновляем город
    if ($up == true) {//если верно, то обновляем его в сессии
...
////////Изменение города
    ...
    $up = mysqli_query($conn,"UPDATE users SET address='$address' WHERE id='$id_user'");//обновляем город
    if ($up == true) {//если верно, то обновляем его в сессии
...
////////Изменение числа, месяца и года
    ...
    $up = mysqli_query($conn,"UPDATE users SET birthdate_day='$birthdate_day',birthdate_month='$birthdate_month',birthdate_year='$birthdate_year' WHERE id='$id_user'");//обновляем день
    if ($up == true) {
        echo "<meta http-equiv='Refresh' content='0; URL=profile.php?id=".$id_user."'>";
    }
}
...
////////Изменение пола
    ...
    $up = mysqli_query($conn,"UPDATE users SET sex='$sex' WHERE id='$id_user'");//обновляем пол
    if ($up == true) {
        echo "<meta http-equiv='Refresh' content='0; URL=profile.php?id=".$id_user."'>";
    }
}
